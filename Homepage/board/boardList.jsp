...
<%
	...
	try {
		...
		// 게시물의 총 수를 얻는 쿼리 실행
		Prepared_state = connect.prepareStatement("SELECT COUNT(NUM) AS TOTAL FROM BOARD" + whereSQL);
		if (!"".equals(whereSQL)) {
			if ("ALL".equals(searchType)) {
		...
		// 게시물 목록을 얻는 쿼리 실행
		Prepared_state = connect.prepareStatement("SELECT NUM, SUBJECT, WRITER, AREA, PERIOD, REG_DATE, HIT FROM BOARD "+whereSQL+" ORDER BY NUM DESC LIMIT ?, ?");
		if (!"".equals(whereSQL)) {
			// 전체검색일시
			if ("ALL".equals(searchType)) {
				Prepared_state.setString(1, searchTextUTF8);
	...
	<table border="1" summary="게시판 목록">
		...
		<tbody>
			...
			<tr>
				...
				<td>&nbsp;<a href="boardView.jsp?num=<%=result.getInt("NUM")%>&amp;pageNum=<%=pagenum_temp%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>"><%=result.getString("SUBJECT") %></a></td>
		...
		<tfoot>
			<tr>
				...
				if(total_count > 0) {
					...
					if(pagenum_temp > 1){
						strBuffer.append("<a href=\"").append("boardList.jsp?pageNum=1&amp;searchType="+searchType+"&amp;searchText="+searchText);
						strBuffer.append("\" title=\"<<\"><<</a>&nbsp;");
					}
					if (isPrev) {
						int goPrevPage = startPage - page_per_block;			
						strBuffer.append("&nbsp;&nbsp;<a href=\"").append("boardList.jsp?pageNum="+goPrevPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
						strBuffer.append("\" title=\"<\"><</a>");
					} else {
					...
					for (int i = startPage; i <= endPage; i++) {
						if (i == pagenum_temp) {
							...
							strBuffer.append("<a href=\"").append("boardList.jsp?pageNum="+i+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
					...
					if (isNext) {
						...
						strBuffer.append("<a href=\"").append("boardList.jsp?pageNum="+goNextPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
						strBuffer.append("\" title=\">\">></a>");
					} else {
						
					}
					if(totalNumOfPage > pagenum_temp){
						strBuffer.append("&nbsp;&nbsp;<a href=\"").append("boardList.jsp?pageNum="+totalNumOfPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
						strBuffer.append("\" title=\">>\">>></a>");
					}
