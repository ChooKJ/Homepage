	...
	try {
		...
		if ("W".equals(mode)) {
			Prepared_state = connect.prepareStatement(
				"INSERT INTO BOARD (SUBJECT, WRITER, CONTENTS, IP, HIT, REG_DATE, MOD_DATE, AREA, PERIOD, LATITUDE, LONGTITUDE) "+
				"VALUES (?, ?, ?, ?, 0, NOW(), NOW(), ?, ?, ?, ?)");
			...
			Prepared_state = connect.prepareStatement(
				"UPDATE BOARD SET SUBJECT = ?, WRITER = ?, CONTENTS = ?, IP = ?, MOD_DATE = NOW(), AREA = ?, PERIOD = ?, LATITUDE = ?, LONGTITUDE = ?"+
				"WHERE NUM = ?");
			Prepared_state.setString(1, subject);
			Prepared_state.setString(2, writer);
			Prepared_state.setString(3, contents);
			...
			Prepared_state = connect.prepareStatement("DELETE FROM BOARD WHERE NUM = ?");
