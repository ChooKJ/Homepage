...
<%
	...
	try {
		...
		// 조회수 증가 쿼리 실행
		Prepared_state = connect.prepareStatement("UPDATE BOARD SET HIT = HIT + 1 WHERE NUM = ?");
		Prepared_state.setString(1, num);
		Prepared_state.executeUpdate();
		// 게시물 상세 조회 쿼리 실행 
		Prepared_state = connect.prepareStatement(
			"SELECT NUM, SUBJECT, CONTENTS, WRITER, HIT, REG_DATE, AREA, PERIOD, LATITUDE, LONGTITUDE FROM BOARD "+ 
			"WHERE NUM = ?");
		Prepared_state.setString(1, num);
